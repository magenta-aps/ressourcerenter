...
class StatistikBaseView(FormView):
    ...
    def get_queryset(self, form, only_fields=None):
        ...
        qs = qs.annotate(
            indberetningstype=Case(
                When(
                    indberetning__skematype__id=1, then=Value("Eksport")
        ...
        if cleaned_data["indhandlingssted"]:
            ...
            qs = qs.filter(
                Q(indhandlingssted__in=cleaned_data["indhandlingssted"])
                | ~Q(indberetningstype="Indhandling")
            )
        ...
        if cleaned_data["fiskeart_eksport"] or cleaned_data["fiskeart_indhandling"]:
            ...
            if cleaned_data["fiskeart_eksport"]:
                qs = qs.annotate(
                    fiskeart_eksport=Case(
                        When(
                            indberetning__skematype__id=1,
                            then="produkttype__fiskeart__navn_dk",
                        ),  # output-værdi under kolonnen 'fiskeart_eksport'
                        default=Value(""),
            ...
            if cleaned_data["fiskeart_indhandling"]:
                qs = qs.annotate(
                    fiskeart_indhandling=Case(
                        When(
                            ~Q(indberetning__skematype__id=1),
                            then="produkttype__fiskeart__navn_dk",
                        ),  # output-værdi under kolonnen 'fiskeart_indhandling'
                        default=Value(""),
                        output_field=TextField(),
                    )
                )
            ...
            qs = qs.filter(
                fiskeart_export_q | fiskeart_indhandling_q
            )  # match export query OR indhandling query

        if cleaned_data["produkttype_eksport"]:
            ordering_fields["produkttype"] = "produkttype__ordering"
            qs = qs.annotate(
                produkttype_eksport=Case(
            ...
            qs = qs.filter(
                Q(produkttype__in=cleaned_data["produkttype_eksport"])
                | ~Q(indberetning__skematype__id=1)  # match produkttype OR not export
            )
...
class StatistikView(ExcelMixin, StatistikBaseView):
    ...
    def get_resultat(self, form):
        ...
        qs = (
            ...
            .order_by(*ordering_fields.values())
        )
        ...
        for db_row_dict in qs:
            ...
            for enhed in enheder:
                ...
                value = value / 1000
                row.append(
                    {"value": value, "floatformat": "1g", "excel_format": "#,##0.0"}
                )

            result.append(row)

        return {"headings": headings, "rows": result}
    ...
    def produkt_fiskeart_map(self):
        return {
            str(fiskeart.uuid): [
                str(produkttype.uuid) for produkttype in fiskeart.produkttype_set.all()
            ]
    ...
    def get_context_data(self, **kwargs):
        return super().get_context_data(
            **{**kwargs, "produkt_fiskeart_map": self.produkt_fiskeart_map}
        )

    def form_valid(self, form, *args, **kwargs):
        context = self.get_context_data(form=form)
        context.update(
            {"form_is_valid": True, "resultat": self.get_resultat(form), "form": form}
        )
...
class StatistikChoicesView(StatistikBaseView):
    ...
    def get_data(self, form, field, fields, filter=None):
        ...
        return [str(value) if value is not None else None for value in qs]
    ...
    def form_valid(self, form):
        ...
        is_not_indhandling = (
            len(indberetningstype) > 0 and "Indhandling" not in indberetningstype
        )
        is_not_eksport = (
            len(indberetningstype) > 0 and "Eksport" not in indberetningstype
        )
        ...
        if not form.cleaned_data["virksomhed"]:
            data["virksomhed"] = self.get_data(
                form, "indberetning__virksomhed", considered_fields
            )
        ...
        if not form.cleaned_data["indberetningstype"]:
            data["indberetningstype"] = self.get_data(
                form, "indberetningstype", considered_fields
            )
        ...
        if is_not_indhandling:
            ...
            data["fiskeart_indhandling"] = self.get_data(
                form, "produkttype__fiskeart__uuid", considered_fields, indhandling
            )
        ...
        if is_not_eksport:
            ...
            data["fiskeart_eksport"] = self.get_data(
                form, "produkttype__fiskeart__uuid", considered_fields, eksport
            )
        ...
        if is_not_eksport:
            ...
            data["produkttype_eksport"] = self.get_data(
                form, "produkttype__uuid", considered_fields, eksport
            )
        ...
        if is_not_indhandling:
            ...
            data["indhandlingssted"] = self.get_data(
                form, "indhandlingssted__uuid", considered_fields
            )

        all_fiskearter = set(
            list(form.cleaned_data["fiskeart_eksport"])
            + list(form.cleaned_data["fiskeart_indhandling"])
        )
        all_value_choices = [
            value for value, label in StatistikForm().fields["enhed"].choices
        ]
        pelagisk_exception = {
            "omsætning_tkr",
            "transporttillæg_tkr",
        }  # Disse felter skal skjules hvis der kun er valgt pelagiske fiskearter
