...
class Prisme(object):
    ...
    def process_service(self, request_object, context, cvr):
        try:
            ...
            logger.info("CVR=%s Sending to %s:\n%s" % (cvr, request_object.method, request_object.xml))
            ...
            for reply_item in reply.instanceCollection.GWSReplyInstanceDCFUJ:
                if reply_item.replyCode == 0:
                    logger.info("CVR=%s Receiving from %s:\n%s" % (cvr, request_object.method, reply_item.xml))
            ...
            logger.info("CVR=%s Error in process_service for %s: %s" % (cvr, request_object.method, str(e)))
            raise e

    def get_account_data(self, cvr, date_from, date_to):
        return self.process_service(PrismeSELAccountRequest(cvr, date_from, date_to), 'account', cvr)
